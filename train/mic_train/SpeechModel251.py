...
class ModelSpeech(): # 语音模型类
	def __init__(self, datapath):
		...
		'''
		MS_OUTPUT_SIZE = 1424 # 即网络输出为1424
		self.MS_OUTPUT_SIZE = MS_OUTPUT_SIZE # 神经网络最终输出的每一个字符向量维度的大小
		#self.BATCH_SIZE = BATCH_SIZE # 一次训练的batch
		self.label_max_string_length = 64
		self.AUDIO_LENGTH = 1600 # 设置语音的长度为1600
		self.AUDIO_FEATURE_LENGTH = 200 # 设置语音特征的长度为200
		self._model, self.base_model = self.CreateModel() # 初始化卷积神经网络，返回model和model_data
	...
	def CreateModel(self):
		...
		input_data = Input(name='the_input', shape=(self.AUDIO_LENGTH, self.AUDIO_FEATURE_LENGTH, 1)) # shape = (1600, 200, 1)
	...
	def Predict(self, data_input, input_len):
		...
		x_in_txt = open('tensor_txt/x_in.txt', 'w')
		x_in_txt.write(str(x_in))
		x_in_txt.write(str(x_in.shape[0]))
		x_in_txt.write(str(x_in.shape[1]))
		x_in_txt.write(str(x_in.shape[2]))
		x_in_txt.write(str(x_in.shape[3]))
		x_in_txt.close()
		...
		#base_pred = keras_model.predict(x_in)
	...
	def RecognizeSpeech(self, wavsignal, fs):
		...
		tensor_txt = open('tensor_txt/tensor_txt.txt', 'w')
		tensor_txt.write(str(data_input))
		tensor_txt.close()
	...
	def RecognizeSpeech_FromFile(self, filename):
		...
		wavsignal,fs = read_wav_data(filename) # 识别语音的特征
...
if(__name__=='__main__'):
	...
	ms.TrainModel(datapath, epoch = 200, batch_size = 16, save_step = 500)
